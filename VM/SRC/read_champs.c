...
/*   Updated: 2023/02/19 12:57:07 by briffard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	size_check(t_vm *vm, int *i, int ret)
{
	if (vm->champs[*i].code_size != \
	ret - 16 - PROG_NAME_LENGTH - COMMENT_LENGTH)
		error_exit("Wrong size", vm);
	if (vm->champs[*i].code_size > CHAMP_MAX_SIZE)
		error_exit("Source code size exceeds the limit", vm);
}

static void	parse_champs(t_vm *vm, int *i, unsigned char *buf, int ret)
{
	int	offset;
